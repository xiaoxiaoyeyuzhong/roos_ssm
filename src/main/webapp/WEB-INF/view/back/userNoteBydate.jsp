			...
			<div class="row">
				...
				<div class="col-sm-10">
					...
					<div class="form-group form-inline">
						...
						<a class="btn btn-primary" href="findCongziNote?page=1"style="margin-left:40px;color:white;">查找所有 </a>
					</div>
					<table class="table">
						<tr>
							<td>编号</td>
						...
						<tr class="sa">
							...
							<td>${n.user.id}</td>
							<td>${n.user.phone}</td>
							<td style="color:red;"><span class="record">${n.record_bill}</span>元</td>
							<td>${n.create_time}</td>
							<td style="color:red;">${n.user.amount}元</td>
							<td>
								<a href="findNoteByPage?openid=${n.openid}&page=1">账单详情</a>
							</td>
						</tr>
						</c:forEach>
					</table>
		...
		<script>
			...
			function submit(){
				...
				if(startDate.length==0){
					...
					window.location.href="findAlljiluBydate?page=1&startDate="+startDate
				}
			}
			...
			function pre(){
				...
				if(page==1){
					...
					$("#num").text(page-1);
					page = $("#num").text();
					$.post("findcongzinoteByDateofPage",{page:page,startDate:"${startDate}"},function(data){
						var str;
						$(".sa").remove();
						var j = (page-1)*13;
						for(var i=0;i<data.length;i++){
							var str =str+"<tr class='sa'>"
											...
											+"<td style='color:red;'>"+data[i].record_bill+"</td>"
											+"<td>"+data[i].create_time+"</td>"
											+"<td style='color:red;'>"+data[i].user.amount+"</td>"
											+"<td>"
												+"<a href='findNoteByPage?openid="+data[i].openid+"&page=1'>账单详情</a>"
											+"</td>"
										+"</tr>";
						}
