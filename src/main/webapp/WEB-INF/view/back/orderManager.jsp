	...
	<body>
				...
				<div class="col-sm-10">
					...
					<table class="table">
						...
						<tr class="infos">
							...
							<td>
								<a href="openOrderContent?orderid=${o.orderid}&page=1">订单详情</a> \ 
								<a href="queryUserInfo?openid=${o.openid}" style="color:black;">客户信息</a>
							</td>
							<td>
								<button class="btn btn-danger" onclick='changeState("${o.orderid}","${o.openid}")'>确定消费</button>
							</td>
						</tr>
		...
		<script>
			...
			function changeState(id,openid){
				if(confirm("要确定消费吗")){
					$.post("OKOrderState",{orderid:id,openid:openid},function(data){
						if(data=="1"){
							alert("已成功确定消费!");
							window.location.href="openOrderManager?page=1&isexpend=未消费";
						}
						if(data=="0"){
							alert("确定不成功请重新确定!");
						}
			...
			function pre(){
				...
				if(page==1){
					...
					$("#num").text(page-1);
					page = $("#num").text();
					$.post("queryOrdersTurnPagesByIsexpend",{page:page,isexpend:"未消费"},function(data){
						var str;
						$(".infos").remove();
						var j = (page-1)*10;
						for(var i=0;i<data.length;i++){
							var str =str+"<tr class='infos'>"
											...
											+"<td>"
												+"<a href='openOrderContent?orderid="+data[i].orderid+"&page=1'>订单详情</a> \ "
												+"<a href='queryUserInfo?openid="+data[i].openid+"' style='color:black;'>客户信息</a>"
											+"</td>"
											+"<td>"
												+"<button class='btn btn-danger' onclick='changeState('"+data[i].orderid+"',+'"+data[i].openid+"')'>确定消费</button>"
											+"</td>"
										+"</tr>;"
						}
