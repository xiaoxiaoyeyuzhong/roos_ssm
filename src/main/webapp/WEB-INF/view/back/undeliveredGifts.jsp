...
<html>
	...
	<body>
					...
					<table class="table">
						...
						<tr class="infos">
							...
							<td class="aa">
								...
								<a href="javascript:changeorder('${go.id}')" class="btn btn-danger" style="color:white;">确定兑换</a>
							</td>
						</tr>
						</c:forEach>
					</table>
		...
		<script>
			...
			if(hao=="ok"){
				alert("已成功兑换");
				window.location.href="queryDeliveredGifts?page=1"
			}
			var y = "${giftordersize}";
			if(y=="0"){
				$(".table tr").hide();
			...
			function changeorder(id){
				if(confirm("确定要兑换了吗？")){
					window.location.href="updateGiftOrderstate?id="+id
				}
			}
			...
			function pre(){
				...
				if(page==1){
					...
					$("#num").text(page-1);
					page = $("#num").text();
					$.post("querylipinOrderPage",{page:page},function(data){
						var str;
						$(".infos").remove();
						var j = (page-1)*8;
						for(var i=0;i<data.length;i++){
							var str =str+"<tr class='infos'>"
											...
											+"<td class='aa'>"
												...
												+"\&nbsp;<a href='javascript:changeorder("+data[i].id+")' class='btn btn-danger' style='color:white;'>确定兑换</a>"
											+"</td>"
										+"</tr>"
						}
